...
def process_key(key):
    ...
    for i in range(1989, 2020):
        file_path = os.path.join("/home/mafzhang/data/cmoms", 
                                    "{}/{}-{}-{}.npy".format(i, i, month, day))
        climatology.append(np.load(file_path))

    climatology = np.stack(climatology, 0)
    climatology = np.nanmean(climatology, 0)
    np.save("/home/mafzhang/data/cmoms/climatology/{}-{}.npy".format(month, day), climatology)
    return climatology

# 主函数
def main(keys):
    with ThreadPoolExecutor(max_workers=8) as executor:  # 设置线程池的最大线程数
        ...
        for future in tqdm(as_completed(futures), total=len(keys)):  # 使用 tqdm 显示进度
            try:
                _ = future.result()  # 获取结果
            except Exception as e:
                print(f"Error processing key {futures[future]}: {e}")
    return None

# 示例调用
keys = list(pd.date_range(start="20190101", end="20191231", freq='D'))
_ = main(keys)